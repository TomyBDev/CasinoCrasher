...
USTRUCT()
struct CASINOCRASHER_API FAbilityMeshMontage
{
	GENERATED_BODY()
...
public:
	...
	ECCAbilityInputID AbilityInputID = ECCAbilityInputID::None;

	// Value to associate an ability with an slot without tying it to an automatically activated input.
	// Passive abilities won't be tied to an input so we need a way to generically associate abilities with slots.
	UPROPERTY(BlueprintReadOnly, EditAnywhere, Category = "Ability")
	ECCAbilityInputID AbilityID = ECCAbilityInputID::None;
	...
	TMap<FGameplayTag, FCCGameplayEffectContainer> EffectContainerMap;
	...
	virtual FCCGameplayEffectContainerSpec MakeEffectContainerSpecFromContainer(const FCCGameplayEffectContainer& Container, const FGameplayEventData& EventData, int32 OverrideGameplayLevel = -1);

	// Search for and make a gameplay effect container spec to be applied later, from the EffectContainerMap
	UFUNCTION(BlueprintCallable, Category = Ability, meta = (AutoCreateRefTerm = "EventData"))
	virtual FCCGameplayEffectContainerSpec MakeEffectContainerSpec(FGameplayTag ContainerTag, const FGameplayEventData& EventData, int32 OverrideGameplayLevel = -1);

	// Applies a gameplay effect container spec that was previously created
	UFUNCTION(BlueprintCallable, Category = "Ability")
	virtual TArray<FActiveGameplayEffectHandle> ApplyEffectContainerSpec(const FCCGameplayEffectContainerSpec& ContainerSpec);
